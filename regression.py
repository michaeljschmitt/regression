...
Instructions:
    1. Please enter your data into a worksheet named 'Sheet' of an Excel workbook entitled 'Data' or 'Data.xlsx'. Running the program will create this for you. 
    The first column is for the dependent variable data. All subsequent columns are for independent variable data. 
    The first row is for header data. Each subsequent row is a data point for your regression.
    2. Add an asterisk symbol to beginning of any header (after the first column) that you wish to exclude from the regression.
    3. Double click the program executible once data is entered into the worksheet to run your regression analysis. 
    Please remember to save workbook and close all related text files before running program. 
...
if (len(y) == 0) and (len(x) == 0):
    ...
    errorFile.write("Worksheet is blank: please enter data.")
    errorFile.close()
    sys.exit()
...
if len(emptyRows) or len(constantCol) or len(largeDevRows) or len(stringCols) > 0 or len(deleteCols) > 0:
    ...
    if len(largeDevRows) > 0:
        cleaningResultsFile.write('------------------------------------------------------------------------------\n**Recommend the following rows be eliminated from dataset as they contain data outside \nacceptable variance (\u03BC > 2\u03C3):\n')
